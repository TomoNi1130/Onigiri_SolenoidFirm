...
uint8_t canid = 128;
//TODOs:intmainの関数にcanidを決める部分を作る。canidはグローバル変数なので、割り込みに付けると信じる
...
int main(void)
{
  ...
  while (1)
  {
    // 受信したデータのうち、配列の0番目をsoleStateに格納する

    // printf("soleState: %d\n", soleState);

    // soleStateのデータをbitごとにGPIOAのピン0から7に出力する
    printf("NowState:%02x\n", soleState);\
    soleState = 0;
    for(int i=0;i<8;i++){
      HAL_GPIO_WritePin(GPIOA, GPIO_PIN_0<<i, (soleState >> i) & 1);
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOA, A_0_Pin|A_1_Pin|A_2_Pin|A_3_Pin
                          |B_0_Pin|B_1_Pin|B_2_Pin|B_4_Pin
                          |CAN_State_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pins : A_0_Pin A_1_Pin A_2_Pin A_3_Pin
                           B_0_Pin B_1_Pin B_2_Pin B_4_Pin
                           CAN_State_Pin */
  GPIO_InitStruct.Pin = A_0_Pin|A_1_Pin|A_2_Pin|A_3_Pin
                          |B_0_Pin|B_1_Pin|B_2_Pin|B_4_Pin
                          |CAN_State_Pin;
